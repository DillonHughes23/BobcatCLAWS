...
import pymysql
import configparser
from wmAPI import read_key, read_db_params, handle_api_response, main
...
def mock_pymysql_connect():
    with patch('wmAPI.pymysql.connect') as mock_connect:
        yield mock_connect

def test_basic_functionality(mock_pymysql_connect):
    ...
    with patch.object(mock_pymysql_connect.return_value, 'commit') as mock_execute_commit:
        ...
        mock_cursor.execute.assert_called_once_with(
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """,
            ('123', 'Product 1', 1, None, None, None, None, 42)
        )
...
def test_main():
    with patch('wmAPI.read_db_params') as mock_read_db_params,\
         patch('wmAPI.pymysql.connect') as mock_pymysql_connect,\
         patch('wmAPI.read_key') as mock_read_key,\
         patch('requests.get') as mock_requests_get,\
         patch('wmAPI.handle_api_response') as mock_handle_api_response,\
         patch('builtins.print') as mock_print,\
         patch('wmAPI.pymysql.connect.close') as mock_close:
        ...
        mock_requests_get.assert_called_once_with('https://api.bluecartapi.com/request', params={'api_key': 'test_key', 'search_term': 'term', 'type': 'search', 'output': 'json'})
